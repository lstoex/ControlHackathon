...
class DroneZHoveringPIDCtrl:
    """
    f (t) = -Kp * (z - target) - Ki * \int (z - target)dt - Kd * vz
    """
    def __init__(self, sampling_time: float, model: droneZModel.DroneZModel):
        self._sampling_time = sampling_time
        self._model = model
        self._ctrl_config = HoveringPIDCtrlConfig()
        self._goal_val = None
        self._error_accumulated = 0.0


    def compute_PID_control(self, x: np.ndarray):
        z_error = x[0] - self._goal_val
        vz = x[1]
        self._error_accumulated += z_error * self._sampling_time
        thrust = self._model.model_config.mass * self._model.model_config.gravity - self._ctrl_config.PID_Kp * z_error  - self._ctrl_config.PID_Kd * vz
...
def main():
    ...
    controller = DroneZHoveringPIDCtrl(sampling_time, model)
    controller.goal = goal
    x_trajectory, u_trajectory = model.simulateClosedLoop(sim_length, x_init, controller.compute_PID_control)
